...
$(function(){
  ...
  function quizShow(){
    ...
    quizArea.find('.quiz_no').text(quiz_cnt + 1);
    // 問題文を表示
    quizArea.find('.quiz_question').text(arryQuiz[quiz_cnt]['question']);
    // 正解の問題を取得する
    var success = arryQuiz[quiz_cnt]['answer'][0];
    // 現在の選択肢表示を削除する
    quizArea.find('.quiz_ans_area ul').empty();
  
    var aryHoge = arryQuiz[quiz_cnt]['answer'];
  ...
  function quizResult(){
    quizArea.find('.quiz_set').hide();
    $text = quiz_cnt +'問中' + quiz_success_cnt +'問正解!';
    if(quiz_fin_cnt === quiz_success_cnt){
      $text += '<br>全問正解おめでとう'; 
      var imgSrc = 'img/lp-mh-vrct-01-1909.jpg';
    ...
    $text += '<br><input class="quiz_restart" type="button" value="もう一度挑戦する">';
    quizArea.find('.quiz_result').prepend($text);
    quizArea.find('.quiz_result img').attr('src',imgSrc);
    quizArea.find('.quiz_result').show();
  }
